...
arguments.callee(match.children);}})(matches);return matches;};Syntax.extractMatches=function(){var rules=arguments;return function(match,expr){var matches=[];for(var i=0;i<rules.length;i+=1){var rule=rules[i];if(rule==null){continue;}
if(rule.debug){alert("'"+match[1]+"' : "+match[1].charCodeAt(0));}
var index=rule.index||(i+1);if(match[index].length>0){if(rule.brush){matches.push(Syntax.brushes[rule.brush].buildTree(match[index],RegExp.indexOf(match,index)));}else{var expression=jQuery.extend({owner:expr.owner},rule);matches.push(new Syntax.Match(RegExp.indexOf(match,index),match[index].length,expression,match[index]));}}}
